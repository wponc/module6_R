...
Uncertainty is the amount of variation possible in an ecological forecast. From a statistical perspective, it is the range of possible outcomes.   
...
**Answer Q.2**
I believe the natural resource managers could adjust their operations according to the uncertainty of a forecast. In that, they would need to be prepared for more diverse scenarios when utilizing forecasts with high uncertainty, as there is a broader range of predicted outcomes. When utilizing forecasts with low uncertainty, they can operate with greater confidence of the events that may happen, and spend less time preparing for all possible outcomes (within a reasonable amount) 
...
Colder water can hold more dissolved oxygen, so increasing water temperatures can lead to lower dissolved oxygen levels. Furthermore, increasing water temperatures can lead to harmful algal blooms, which can further deplete dissolved oxygen levels in water.
...
Yes, there seems to be a reasonably strong relationship between air temperature and water temperature. 
...
Each gray line represents an ensemble member used in the forecasted air temperature. There are 30 ensemble members used for the NOAA weather forecasts, so there are 30 gray lines. 
...
Water temperature is being predicted as a function of two separate input variables: yesterday's water temperature, and today's air temperature. This is where the word 'multiple' comes in for multiple linear regression. The 'linear regression' portion is used to describe the model's linear structure, where each of the input variables is multiplied by a factor. These results are then added to a coefficient to generate water temperature.  
...
The coefficients indicate that past water temperature has a far stronger relationship with current water temperature than current air temperature. Driver variables that have no impact on the predicted values would have a coefficient of zero, while driver variables with a coefficient near 1 would indicate a direct relationship. Given that current air temperature's coefficient is 0.18 and yesterday's water temperature is 0.81, it is safe to conclude that the latter has far greater influence on current water temperature. Given that the standard error values are very low, both below 0.02, we can have strong confidence in our fitted model parameter values.
...
Overall, the model performed very well by achieving an r2 value of 0.9. An R-squared value of 1.0 indicates that the response variable is explained perfectly by the predictor without any error. Furthermore, our error value of 1.34x10-15 may be essentially considered as zero, another indicator of strong performance. Finally, a simple loook at the plot shows very strong correlation between the modeled and observed outputs. 
...
**Answer Q.11** 
The curves (ensemble members) of the NOAA GEFS air temperature forecast experience a much greater variation in their values over the forecasted week. They rise slightly from their starting values of ~27, before dropping dramatically to ~20 near the end of the forecast. Forecasted water temperature likely experiences  slower shift in temperatures because water has greater heat retention ability than air.
...
**Answer Q.12**
Forecast uncertainty increases with time. As time progresses, the inherent uncertainty in our driver variables propagates, growing exponentially larger with each time step. 
...
**Answer Q.13** 
The probabilistic forecast provides a much more informative, realistic prediction of future water temperatures. It acknowledges the variation present in the forecasts, and covers a wide range of possible futures in it's spread. 
...
**Answer Q.14**
We can reduce parameter uncertainty by using a number greater than 30 when generating our parameter distributions. As the size (number of rows) for these distributions increases, they approach normality in their shape. With more normal parameter distributions, parameter undertainty would be reduced in the plot above and the ensemble curves would be closer together.
...
**Answer Q.15**
Initial conditions become more uncertain as the number of observations in our rnorm distribution decreases. This will lead to a less normal distribution for initial conditions. Initial conditions can also become more uncertain with less frequent data collection. 
...
Overall, parameter uncertainty contributes the most to total forecast uncertainty as indicated by the proportion of the amber section in the vertical bars. 
...
**Answer Q.17**
Different water temp forecast models would have different quantities of uncertainty. Most obviously, during objective #8 we generated a distribution of initial conditions. The distribution obtained for initial conditions will be different each time this step is run, as it gathers a random sample of numbers from a normal distribution. As such, no two water temperature forecast models using this method will be exactly identical. 
...
```{r}
fit <- lm(model_data$wtemp ~ model_data$wtemp_yday + model_data$airt + model_data$swr) #INSERT ADDITIONAL COVARIATES IN THE PARENTHESES
...
**Answer Q.19**
Water temperature is now being predicted by incorporating yesterday's water temperature, today's air temperature, and also short wave radiation data. 
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!!!!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$shortwave_radiation * coeffs[4]
